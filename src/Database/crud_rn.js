                ...
                SQLite.openDatabase(database_name, database_version, database_displayname, database_size).then(DB => {
                    ...
                    db.executeSql('SELECT 1 FROM Item LIMIT 1').then(() => {
                        ...
                        db.transaction((tx) => {
                            ...
                            tx.executeSql('CREATE TABLE IF NOT EXISTS Item (id INTEGER PRIMARY KEY AUTOINCREMENT, tarefa varchar(30), descricao  varchar(30), termino varchar(30) , status varchar(30), prioridade varchar(30))');
                        }).then(() => {
                ...
                db.transaction((tx) => {
                    ...
                    tx.executeSql('SELECT * FROM Item', []).then(([tx, results]) => {
                        ...
                        for (let i = 0; i < len; i++) {
                            ...
                            const { id, tarefa, descricao, termino, status, prioridade } = row;
                            lista.push({ id, tarefa, descricao, termino, status, prioridade });
                        }
                        console.log(lista);
                        resolve(lista);
                    });
        ...
        return new Promise((resolve) => {
            this.Conectar().then((db) => {
                db.transaction((tx) => {
                    ...
                    tx.executeSql('INSERT INTO Item (tarefa, descricao, termino, status, prioridade) VALUES (?, ?, ?,?,?)', [item.tarefa, item.descricao, item.termino,item.status,item.prioridade]).then(([tx, results]) => {
                        resolve(results);
                    });
                }).then((result) => {
                    this.Desconectar(db);
